...
%\noindent \textsc{Published by Team PSE2}\\ % Publisher
...
The @POST annotation states the http method. The @Path annotation sets the url path. The method can "consume" data sent by the client, which is in this case a java object of a "DuplicateModel", which is passed with the call as a JSON object and then gets parsed by the server into a POJO. The method "produces" a response, which is the method's return value, in this case also in JSON. 
\\\\
